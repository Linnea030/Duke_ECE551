...
board_t * makeBoard(int w, int h, int numMines) {
  board_t * b = malloc(sizeof(*b));  //WRITE ME!
  if (b == NULL) {
    perror("There is no space for board\n");
    exit(EXIT_FAILURE);
  }
  b->height = h;
  b->width = w;
  ...
  if (numMines < 0) {
    perror("Invalid number of nummines\n");
    exit(EXIT_FAILURE);
  }
  b->board = malloc(h * sizeof(*b->board));  //array for height, each height is a member
  if (b->board == NULL) {
    perror("There is no space for board\n");
    exit(EXIT_FAILURE);
  }
  for (int i = 0; i < h; i++) {
    b->board[i] = malloc(w * sizeof(**b->board));  //each height point to all width
    if (b->board[i] == NULL) {
      perror("There is no space for board\n");
      exit(EXIT_FAILURE);
    }
...
int countMines(board_t * b, int x, int y) {
  //WRITE ME!
  int count = 0;
  if (x > 0 && x < b->width - 1 && y < b->height - 1 && y > 0) {
