...
void Page::print_p(std::string path, int step) const {
  if (step == 1) {
    //print pagenumber
...
void Page::textPrint(std::string path) const {
  //print text
...
void Page::print_step4(std::vector<std::pair<long int, std::string> > storyVar) {
  int n = choice.size();
  for (int i = 0; i < n; ++i) {
